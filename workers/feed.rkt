...
(define/contract (parse-episode ep-xexpr)
  ...
  (define/contract (ep-alist->ep-hash ep-alist)
    ...
    (let loop ((rem ep-alist) (acc (hash)))
      (cond ((null? rem) acc)
            ...
            ((and (string? (hash-ref acc (caar rem)))
                  (string? (cdar rem))
                  (> (string-length (cdar rem)) (string-length (hash-ref acc (caar rem)))))
             (loop (cdr rem) (hash-set acc (caar rem) (cdar rem))))
            ((list? (caar rem))
             (loop (cdr rem) (hash-set acc (caar rem) (remove-duplicates (append (cdar rem) (hash-ref acc (caar rem)))))))
            (else (loop (cdr rem) acc)))))
  (define/contract (parse-duration duration/string)
   ...
   (match ep-xexpr
    ...
    ((list 'title '() content ...)
     (list (cons 'title (string-trim (apply string-append (map parse-text content))))))
    ((list (or 'description 'itunes:summary) '() content ...)
     (list (cons 'description (string-trim (apply string-append (map parse-text content))))))
    ((list 'pubDate '() content ...)
     (list(cons 'date_published (rfc2822->sql-timestampz (string-trim (apply string-append (map parse-text content)))))))
    ((list 'itunes:keywords '() content ...)
     (list (cons 'tags (string-split (string-trim (apply string-append (map parse-text content)) ",")))))
    ((list 'itunes:duration '() content ...)
     (list (cons 'duration (parse-duration (string-trim (apply string-append (map parse-text content)))))))
    ((list 'itunes:author '() content ...)
      (list (cons 'author  (string-trim (apply string-append (map parse-text content))))))
...
(define/contract (parse-text an-xexpr)
  (-> x:xexpr? string?)
  (define/contract (handle-cdata cdata/string) ;remove CDATA wrapper and tab characters
    (-> string? string?)
    (let ((inner-text (second (regexp-match #px"^<!\\[CDATA\\[(.*)\\]\\]>$" (string-trim cdata/string)))))
      (apply string-append (string-split inner-text "\t"))))
  (match an-xexpr
    ((struct x:cdata (_ _ text))
     (handle-cdata text))
    ((pregexp #px"^\\s*$") "")
    ((? string? text) text)
